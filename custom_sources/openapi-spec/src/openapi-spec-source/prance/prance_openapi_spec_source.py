...
class OpenApiSpecSource(Source):
    ...
    def set_metadata(self, dataset_name: str, schema: dict, platform: str = "api"
    ) -> SchemaMetadata:
    ...
    def get_workunits_internal(self) -> Iterable[MetadataWorkUnit]:
        ...
        # Adapt for Prance        
        parser = ResolvingParser(config.path, 
                                 recursion_limit=1, 
                                 strict=False, 
                                 recursion_limit_handler=self.recursion_limit_handler_none)
        url_endpoints = self.get_endpoints(parser.specification)
        # to here
